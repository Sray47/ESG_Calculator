...
function renderPrinciple7(doc, essentialData, leadershipData, calculatedP7Data, currentY, contentWidth, pageMargin) {
    ...
    if (essentialData.trade_associations_affiliations && essentialData.trade_associations_affiliations.length > 0) {
        ...
        currentY = renderKeyValue(doc, '1. Trade Associations Affiliations', 'No affiliations listed.', pageMargin, currentY, 350, contentWidth - 350);
    }
    currentY += 5;

    if (essentialData.public_policy_advocacy_details && essentialData.public_policy_advocacy_details.length > 0) {
        ...
        const policyAdvocacyTable = {
            ...
            rows: essentialData.public_policy_advocacy_details.map(p => [p.subject || 'N/A', p.method || 'N/A', p.alignment_with_ndc || 'N/A'])
        };
    ...
    currentY += 5;
    currentY = renderKeyValue(doc, "3. Details of anti-competitive conduct proceedings", essential.anti_competitive_conduct_proceedings_details, startX, currentY, 350, contentWidth - 350, { isLongText: true });

    if (leadershipData) {
        currentY = checkAndAddPage(doc, currentY);
        currentY = addSubHeading(doc, "Leadership Indicators", currentY);
        currentY = renderKeyValue(doc, "a. Additional public policy advocacy details", leadership.additional_public_policy_advocacy_details || 'N/A', startX, currentY, 350, contentWidth - 350, { isLongText: true });
        currentY = renderKeyValue(doc, "b. Consistency of policy advocacy with sustainability goals", leadership.advocacy_consistency_with_sustainability_goals || 'N/A', startX, currentY, 350, contentWidth - 350, { isLongText: true });
        currentY = renderKeyValue(doc, "c. Board review frequency of policy advocacy", leadership.board_review_frequency_policy_advocacy || 'N/A', startX, currentY, 350, contentWidth - 350);
    }
    doc.y = currentY;
    return doc.y;
}
