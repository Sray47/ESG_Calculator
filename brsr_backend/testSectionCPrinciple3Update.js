// A test script that directly updates the Section C Principle 3 data in the database
const { pool } = require('./db');

async function testSectionCPrinciple3Update() {
  try {
    ...
    // Create test data for Section C Principle 3
    const testData = {
      ...
      leadership_indicators: {
        // Add leadership indicator test data if needed
      }
    };
    
    console.log('\nUpdating sc_p3_employee_wellbeing with test data:');
    console.log(JSON.stringify(testData, null, 2));
    
    // Update the database directly
    const updateQuery = `
      UPDATE brsr_reports 
      SET sc_p3_employee_wellbeing = $1 
    ...
    if (updateResult.rows.length > 0) {
      console.log('\nSUCCESS: Updated Section C Principle 3 data!');
      ...
      const verifyQuery = `
        SELECT sc_p3_employee_wellbeing FROM brsr_reports 
        WHERE id = $1
      `;
      ...
      console.log(JSON.stringify(verifyResult.rows[0].sc_p3_employee_wellbeing, null, 2));
    } else {
